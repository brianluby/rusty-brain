...
- **Multi-file `apply_patch` patches record every touched path**: one
  `apply_patch` call can add/update/delete several files in a single V4A
  patch (the live capture proves it); capture now records one observation per
  `*** <op> File:` directive — in patch order, deduplicated — instead of only
  the first, for Codex and OpenCode alike. Malformed/non-V4A payloads still
  fail open to a single `unknown` file touch.
