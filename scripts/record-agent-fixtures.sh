...
# per-agent recorder state. codex is fully isolated: the recorder home holds
# persisted trust + a READ-ONLY copy of ~/.codex/auth.json (mode 0600), and the
# real ~/.codex is never mutated. opencode is the documented exception: it
# records under the operator's REAL ~/.config/opencode (so its default config
# picks a working model), writing ONE session entry to real opencode history;
# no auth/config file is copied or modified, and committed fixtures are sanitized.
set -euo pipefail
...
for h in homes:
    ...
    # Some payloads serialize paths without the leading slash (opencode's
    # relativePath / "Users/<name>/..."), which the exact-string replace above
    # misses and would leak the operator username. Rewrite that form too.
    if h.startswith("/"):
        data = data.replace(h.lstrip("/"), "Users/user")
subs = [
...
emit_readme() {
  ...
  # opencode does not).
  local trust_line record_line home_line
  case "$agent" in
    codex)
...
setup_trust() { # agent
  ...
  case "$agent" in
    ...
    opencode)
      ...
      # Fail closed: this is the only opencode preflight. A passing exit here is
      # the signal recording can proceed; swallowing it would push auth/config
      # errors into the later record step.
      if ! "$cli" auth list; then
        echo "ERROR: opencode auth is not configured for the current user (\`opencode auth login\`)." >&2
        return 1
      fi
...
self_test() {
  ...
  # Slash-stripped home (e.g. opencode's relativePath "Users/<name>/...") must be
  # rewritten too, or the operator username leaks into committed fixtures.
  local sstrip
  sstrip="$(printf 'relativePath:Users/realuser/proj/notes.txt' | scrub /Users/realuser)"
  check "scrub rewrites slash-stripped home" "1" "$(printf '%s' "$sstrip" | grep -cF 'Users/user/proj')"
  check "scrub leaks no slash-stripped home" "0" "$(printf '%s' "$sstrip" | grep -cF 'Users/realuser')"
