...
mod tests {
    ...
    fn event_needs_daemon_is_true_for_all_store_events() {
        ...
        // connection. If a new store event is added, it must appear here.
        let store_events = [
            HookEvent::SessionStart {
    ...
    fn event_needs_daemon_is_false_for_all_local_only_events() {
        // Local-only events skip the connect cost; none of them read or write
        // the store. Verifying all of them in one place ensures a future
        // classification change is explicit and reviewed.
        let local_events = [
